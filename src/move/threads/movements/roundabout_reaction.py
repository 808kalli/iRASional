...
def lane_det(bird_view, original_image, Minv):
    ...
    for window in range(nwindows):
        ...
        if (right_lane_warning == 0):
            ...
            if len(good_right_inds) > minpix:
                if rightx_current > int(np.mean(nonzerox[good_right_inds])):
    ...
    if (right_lane_warning == 0):
        ...
        # right_fit = np.polyfit(righty, rightx, 6)
        # right_fitx = right_fit[0]*ploty**6 + right_fit[1]*ploty**5 + right_fit[2]*ploty**4 + right_fit[3]*ploty**3 + right_fit[4]*ploty**2 + right_fit[5]*ploty + right_fit[6]
